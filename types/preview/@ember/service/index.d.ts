declare module '@ember/service' {
  ...
  export function inject<K extends keyof Registry & string>(
    name: K
  ): InstanceType<Registry[K]> & BasicComputedProperty; // @inject('store') foo      @inject() foo
  ...
  export function service<K extends keyof Registry & string>(
    name: K
  ): InstanceType<Registry[K]> & BasicComputedProperty; // @service('store') foo      @service() foo

  type AnyService = abstract new (...args: any[]) => Service;

  // A type registry for Ember `Service`s. Meant to be declaration-merged so
  // string lookups resolve to the correct type.
  interface Registry extends Record<string, AnyService> {}
}
